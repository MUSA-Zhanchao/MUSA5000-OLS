...
After obtaining a general understanding of the data, we conduct multiple regression analysis to examine the relationship between the dependent variable, **Median House Value (MEDHVAL)**, and the predictors: **education attainment (PCTBACHMOR)**, **number of Households Living in Poverty (NBELPOV100)**, **percentage of Vacant Houses (PCTVACANT)**, and **percentage of Single House Units (PCTSINGLES)**. Regression analysis is a statistical method used to examine the relationship between a dependent variable and one or more predictors, allowing researchers to determine both the strength and direction of these relationships, make predictions, and assess the statistical significance of each predictor. The model also estimates coefficients for each predictor, which represent the expected change in the dependent variable for a one-unit change in the predictor, holding other predictors constant. For this study, the multiple regression model is formulated as follows:
>>>>>>> 6ab4852bf8125a654a8bb65a99629f1a1b335707
$$
...
<<<<<<< HEAD
For the results of the regression analysis to be valid, several key assumptions must be met.  These assumptions including **linearity**, **independence of observations**, **homoscedasticity**, **normality of residuals**, **no multicollinearity**, and **no fewer than 10 observations per predictors**.
=======
...
<<<<<<< HEAD
After verifing the assumptions of the regression, we start the regression analysis. Wh estimate the following parameters: 
- \( \beta_0 \), which is the intercept
- \( \beta_1, \dots, \beta_k \), which are the coefficients of each independent variables
=======
